...
let apply_tac ~token ~tac { Common.ThmDecl.names; node } =
  ...
  let st, feedback =
    ...
    | Coq.Protect.E.{ r = Interrupted; feedback } ->
      ...
      let msg =
        (None, derror, Pp.(str "Coq was interrupted while executing the tactic"))
      in
      (st, msg :: feedback)
    | Coq.Protect.E.{ r = Completed (Error (User (loc, e))); feedback }
    | Coq.Protect.E.{ r = Completed (Error (Anomaly (loc, e))); feedback } ->
      let msg = (loc, derror, e) in
      (st, msg :: feedback)
    | Coq.Protect.E.{ r = Completed (Ok None); feedback } ->
      let msg = (None, derror, Pp.(str "EOF when parsing tactic")) in
      (st, msg :: feedback)
