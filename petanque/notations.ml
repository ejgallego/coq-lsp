...
let warn_inconsistent_scope =
  CWarnings.create ~name:"my-inconsistent-scopes" ~category:CWarnings.CoreCategories.syntax
...
let warn_shadowed_implicit_name =
  CWarnings.create ~name:"my-shadowed-implicit-name" ~category:CWarnings.CoreCategories.syntax
...
let warn_ignoring_unexpected_implicit_binder_declaration =
  CWarnings.create ~name:"my-unexpected-implicit-declaration" ~category:CWarnings.CoreCategories.syntax
...
let save_and_intern_notation save intern env ntnvars loc ntn fullargs =
  ...
  save (List.map Loc.make_loc posl) (Names.DirPath.to_string path) (Names.DirPath.to_string secpath) (snd ntn) sc;
...
let warn_nonprimitive_projection =
  CWarnings.create ~name:"my-nonprimitive-projection-syntax" ~category:CWarnings.CoreCategories.syntax ~default:CWarnings.Disabled
...
let list_notations_and_internalize globalenv env pattern_mode (_, ntnvars as lvar) c =
  let ntnl = ref [] in
  ...
  let rec intern env = CAst.with_loc_val (fun ?loc -> function
    ...
    | CNotation (_,ntn,args) ->
        let c = save_and_intern_notation save_ntn intern env ntnvars loc ntn args in
        apply_impargs env loc c []
    | CGeneralization (b,c) ->
    ...
    | CApp (f, args) ->
        ...
        | CNotation (_,ntn,ntnargs) ->
          let c = save_and_intern_notation save_ntn intern env ntnvars loc ntn ntnargs in
...
let interp_context_evars_gen ?(program_mode=false) ?(unconstrained_sorts = false) ?(impl_env=empty_internalization_env) ?(share=false) ?(autoimp_enable=true) ~dump env sigma make_decl push_decl bl =
  ...
  let ids =
    (* We assume all ids around are parts of the prefix of the current
     ...
     extract_ids env in
  let int_env = default_internalization_env ids (Evd.universe_binders sigma) impl_env in
...
let list_notations_in_statement_lemma ~program_mode env0 evd thm =
  let (_, (bl, t)) = thm in
  let evd, (impls, ((env, _), _)) = interp_context_evars ~program_mode env0 evd bl in
  let intern_env = { ids = extract_ids env
                   ; strict_check = None
                   ...
                   ; impls
                   ; binder_block_names = Some (Some AbsPi)
                   ; ntn_binding_ids = Id.Set.empty } in
  snd (list_notations_and_internalize env intern_env false (empty_ltac_sign, Id.Map.empty) t)
...
let list_notations_in_statement_expr ~atts c =
  ...
  | VernacSynPure x ->
    list_notations_in_statement_vernac ~atts x
...
let vernac_pperr_endline = CDebug.create ~name:"list_notations_in_statement" ()

let list_notations_in_statement ~intern ~st cmd =
  ...
  Flags.verbosely list_notations_in_statement_control entry
...
module Notation = struct
  ...
  let loc_to_yojson loc : Yojson.Safe.t = `Assoc ([
    ("fname", match loc.Loc.fname with
      | InFile { dirpath ; file } -> `String (Option.cata (fun dp -> dp ^ "/" ^ file) file dirpath)
      | ToplevelInput -> `Null);
    ("line_nb", `Int loc.line_nb);
    ("bol_pos", `Int loc.bol_pos);
    ("line_nb_last", `Int loc.line_nb_last);
    ("bol_pos_last", `Int loc.bol_pos_last);
    ("bp", `Int loc.bp);
    ("ep", `Int loc.ep)
  ])

  let loc_of_yojson (t : Yojson.Safe.t) = match t with
    | `Assoc [("fname", `Null);
              ("line_nb", `Int line_nb);
              ("bol_pos", `Int bol_pos);
              ("line_nb_last", `Int line_nb_last);
              ("bol_pos_last", `Int bol_pos_last);
              ("bp", `Int bp);
              ("ep", `Int ep)] ->
      Ok (Loc.{fname = ToplevelInput; line_nb; bol_pos; line_nb_last; bol_pos_last; bp; ep})
    | `Assoc [("fname", `String source);
              ("line_nb", `Int line_nb);
              ("bol_pos", `Int bol_pos);
              ("line_nb_last", `Int line_nb_last);
              ("bol_pos_last", `Int bol_pos_last);
              ("bp", `Int bp);
              ("ep", `Int ep)] ->
      let sources = List.rev (String.split_on_char '/' source) in
      let dirpath, file =
        if List.length sources = 1 then None, source
        else Some (String.concat "/" (List.rev (List.tl sources))), List.hd sources
      in
      Ok (Loc.{fname = InFile {dirpath; file}; line_nb; bol_pos; line_nb_last; bol_pos_last; bp; ep})
    | _ -> Error "Not the right format for a location."

  let rec loc_list_of_yojson = function
    | [] -> Ok []
    | t :: tl ->
      Result.bind (loc_of_yojson t) (fun loc -> Result.map (List.cons loc) (loc_list_of_yojson tl))

  let to_yojson {locations; path; secpath; notation; scope} : Yojson.Safe.t = `Assoc ([
    ("locations", `List (List.map loc_to_yojson locations));
    ("path", `String path);
    ("secpath", `String secpath);
    ("notation", `String notation);
    ("scope", match scope with Some scope -> `String scope | _ -> `Null)
  ])

  let of_yojson (t : Yojson.Safe.t) = match t with
    | `Assoc [("locations", `List locations);
              ("path", `String path);
              ("secpath", `String secpath);
              ("notation", `String notation);
              ("scope", `Null)] ->
      Result.map (fun locations -> {locations; path; secpath; notation; scope = None}) (loc_list_of_yojson locations)
    | `Assoc [("locations", `List locations);
              ("path", `String path);
              ("secpath", `String secpath);
              ("notation", `String notation);
              ("scope", `String scope)] ->
      Result.map (fun locations -> {locations; path; secpath; notation; scope = Some scope}) (loc_list_of_yojson locations)
    | _ -> Error "Not the right format for a notation."

  let tuple_to_notation (locations, path, secpath, notation, scope) =
    {locations; path; secpath; notation; scope}

end

let coq_list_notations_in_statement ~intern ~st cmd =
  let cmd = Coq.Ast.to_coq cmd in
  let res = list_notations_in_statement ~intern ~st cmd in
  List.map Notation.tuple_to_notation res

let coq_list_notations_in_statement ~token ~intern ~st cmd =
  Coq.Protect.eval ~token ~f:(coq_list_notations_in_statement ~intern ~st) cmd
