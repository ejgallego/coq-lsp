...
The main changes for the 0.2.x series are:

- `serlib` is now part of `coq-lsp`. Thus `coq-lsp` doesn't depend on
  SerAPI anymore. We recommend all SerAPI users to migrate to
  `coq-lsp`, as of today `coq-lsp` + `petanque` offer much improved
  functionality over it.

- SerAPI itself has been put in archival mode [1], we'd like to thank
  all contributors to it!

- Renaming: we will rename `coq-lsp` to `rocq-lsp` in the 0.3.x
  series.

- Windows install: `coq-lsp` can be installed on Windows using the new
  Opam native Windows support, `opam install coq-lsp` will do the trick!

- Supported versions: we support Coq 8.17 to Rocq master. Support for
  Rocq 9.0 is new to 0.2.3.

- `petanque` has seen many improvements, thanks to Guillaume Baudart,
  Marc Lelarge, and Jules Viennot Franca,

- Many other improvements and bugfixes, including quickFix support
  from Rocq upstream, improvements to on-demand mode, memory sharing,
  etc... Of particular importance to some users will be the memory
  savings which allow to work simultaneously with many users (joint
  work with Gaëtan Gilbert)

- We will be around for the Rocq'n'share workshop [2], don't hesitate
...
Listochkin, Gaëtan Gilbert, Léo Stefanesco, Jules Viennot Franca, as
well as all the authors of Rocq overlays for Rocq's CI system.
