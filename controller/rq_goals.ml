...
let parse_and_execute_in tac st =
  ...
  let* ast = parse tac st in
  match ast with
  | Some ast -> (Fleche.Memo.Interp.eval (st, ast)).res
  (* On EOF we return the previous state, nothing to do *)
  | None -> Coq.Protect.E.ok st

let run_pretac ~st pretac =
  match pretac with
  | None -> Coq.Protect.E.ok st
  | Some tac -> Coq.State.in_stateM ~st ~f:(parse_and_execute_in tac) st
...
let get_goal_info ~doc ~point ~mode ~pretac () =
  ...
  | Some node ->
    ...
    let* st = run_pretac ~st pretac in
